      ...
      async (err, docs) => {
        ...
        if (docs != null) {
          ...
          if (await checkProductExist(req.body.products)) {
            var updateCart = await ShoppingCart.updateOne(
              { customerId: req.body.customerId },
            ...
            if (
              updateCart.acknowledged == true &&
              updateCart.modifiedCount > 0
            ) {
              ...
              updatedCart.products.forEach((element) => {
                subTotal =
                  subTotal +
                  Math.round(
                    parseInt(element.productId.price) * element.quantity
                  );
              });
...
exports.editItemQuantityInCart = async (req, res) => {
  try {
    ...
    if (updateCart.acknowledged && updateCart.modifiedCount > 0) {
      var cart = await ShoppingCart.find({
        customerId: req.body.customerId,
      }).populate({
        path: "products",
        populate: {
...
exports.removeItemFromCart = async (req, res) => {
  ...
  var updatedCart = await ShoppingCart.updateOne(
    {
      customerId: req.body.customerId,
      "products.productId": req.body.productId,
    },
...
function calculateSubTotal(cart) {
  ...
  cart.forEach((element) => {
    ...
    subTotal =
      subTotal +
      Math.round(parseInt(element.productId.price) * element.quantity);
  });
